...
def cmd_validate_story_creation(args: list[str]) -> int:
    ...
    def create_check_payload(story_id: str, state_file: str) -> dict[str, object]:
        ...
        return create_story_artifact(
            project_root=project_root, story_key=story_id, contract=contract
        )
    ...
    def count_reason(created: int, expected: int) -> str:
        if created == expected:
            return (
                ...
                if expected == 1
                else f"Exactly {expected} story files created as expected"
            )
        if created == 0:
    ...
    if action == "check":
        ...
        while idx < len(rest):
            if rest[idx] == "--before":
                ...
                if idx + 1 < len(rest):
                    ...
                    return print_check_error(
                        story_id,
                        reason="--before requires a value",
                        before_count=before_count,
                        after_count=after_count,
                    )
                continue
            ...
            if rest[idx] == "--artifacts-dir":
                ...
                return print_check_error(
                    story_id,
                    reason="--artifacts-dir requires a value",
                    before_count=before_count,
                    after_count=after_count,
                )
            ...
            if rest[idx] == "--state-file":
                ...
                return print_check_error(
                    story_id,
                    reason="--state-file requires a value",
                    before_count=before_count,
                    after_count=after_count,
                )
            before_count, after_count = parsed_delta_counts(before_value, after_value)
            return print_check_error(
                story_id,
                reason=f"unsupported check argument: {rest[idx]}",
                before_count=before_count,
                after_count=after_count,
            )
        if before_seen != after_seen:
            return print_check_error(
                story_id, reason="both --before and --after are required together"
            )
        ...
        try:
            ...
            response = build_check_response(
                story_id, payload, before_count=before_count, after_count=after_count
            )
        except (FileNotFoundError, PolicyError, ValueError) as exc:
            return print_check_error(
                story_id, reason=str(exc), before_count=before_count, after_count=after_count
            )
        print(json.dumps(response, separators=(",", ":")))
        return 0
    ...
    if action and action not in {"count", "check", "list", "prefix"}:
        if not rest:
            return print_check_error(
                action, reason="both --before and --after are required together"
            )
        if len(rest) == 1:
            return cmd_validate_story_creation(["check", action, "--before", rest[0]])
        return cmd_validate_story_creation(
            ["check", action, "--before", rest[0], "--after", rest[1], *rest[2:]]
        )
    ...
    print(
        "  check <story_id> [--state-file PATH]   - Compatibility wrapper for create verifier",
        file=os.sys.stderr,
    )
