...
def _usage(code: int) -> int:
    ...
    print(
        "  verify-step <step> <story_or_epic> [--state-file path] [--output-file path]", file=target
    )
...
def _sprint_status(args: list[str]) -> int:
    ...
    if args[0] == "get":
        ...
        if not status.found:
            ...
            return 0
        print_json(
            {"found": True, "story": status.story, "status": status.status, "done": status.done}
        )
        return 0
    if args[0] == "exists":
    ...
    while idx < len(args):
        if args[idx] == "--set" and idx + 1 < len(args):
            ...
            replaced, count = re.subn(
                rf"(?m)^{re.escape(key)}:.*$", lambda m, k=key, v=value: f"{k}: {v}", text
            )
            if count:
                text = replaced
                updated.append(key)
...
def _escalate(args: list[str]) -> int:
    ...
    if trigger == "review-loop":
        ...
        if cycles >= limit:
            print_json(
                {"escalate": True, "reason": f"Review loop exceeded max cycles ({cycles}/{limit})"}
            )
        else:
...
def _commit_ready(args: list[str]) -> int:
    ...
    if status.done:
        ...
        if out.strip():
            print_json(
                {"ready": True, "story": args[0], "status": "done", "uncommitted_changes": True}
            )
            return 0
...
def _story_file_status(args: list[str]) -> int:
    ...
    if norm is None:
        ...
        return 1
    matches = sorted(
        (Path(get_project_root()) / "_bmad-output" / "implementation-artifacts").glob(
            f"{norm.prefix}-*.md"
        )
    )
    if not matches:
    ...
    print_json(
        {
            ...
            "title": find_frontmatter_value_case(matches[0], "Title"),
        }
    )
    return 0
