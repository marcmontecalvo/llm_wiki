...
def load_bundled_policy(
    project_root: str | None = None, *, resolve_assets: bool = True
) -> dict[str, Any]:
...
def load_runtime_policy(
    ...
    *,
    ...
    if resolved_state:
        ...
        return load_policy_for_state(
            str(state_path), project_root=str(root), resolve_assets=resolve_assets
        )
    return load_effective_policy(str(root), resolve_assets=resolve_assets)
...
def load_policy_snapshot(
    ...
    *,
    ...
    if expected_hash and actual_hash != expected_hash:
        raise PolicyError(
            f"policy snapshot hash mismatch: expected {expected_hash}, got {actual_hash}"
        )
    try:
...
def summarize_state_policy_fields(
    fields: dict[str, Any], *, project_root: str | Path | None = None
...
def resolve_policy_state_file(
    project_root: str | Path | None = None, state_file: str | Path | None = None
) -> tuple[str, str]:
    root = Path(project_root or get_project_root()).resolve()
    ...
    if env_state:
        return str(
            _resolve_state_path(
                root, Path(env_state).expanduser(), allow_outside=False, label="env state file"
            )
        ), "env"
    ...
    if marker.is_file():
        ...
        return str(
            _resolve_state_path(
                root,
                Path(marker_state).expanduser(),
                allow_outside=False,
                label="marker state file",
            )
        ), "marker"
    return "", ""
...
def parser_runtime_config(policy: dict[str, Any]) -> dict[str, object]:
    ...
    if provider not in VALID_PARSER_PROVIDERS:
        raise PolicyError(
            f"runtime.parser.provider must be one of: {', '.join(sorted(VALID_PARSER_PROVIDERS))}"
        )
    if not model:
...
def _apply_legacy_env(policy: dict[str, Any]) -> None:
    ...
    if review_cycles:
        policy.setdefault("workflow", {}).setdefault("repeat", {}).setdefault("review", {})[
            "maxCycles"
        ] = _legacy_env_int(
            "MAX_REVIEW_CYCLES",
            review_cycles,
...
def _validate_policy_shape(policy: dict[str, Any]) -> None:
    ...
    if (
        "snapshot" in policy
        and "relativeDir" in snapshot
        and not isinstance(snapshot.get("relativeDir"), str)
    ):
    ...
    for name, contract in steps.items():
        ...
        if not isinstance(required, list) or any(
            item not in VALID_ASSET_NAMES for item in required
        ):
...
def _resolve_policy_paths(policy: dict[str, Any], *, project_root: Path, bundle_root: Path) -> None:
    for name, contract in (policy.get("steps") or {}).items():
        ...
        prompt["templatePath"] = _resolve_data_path(
            template_file, project_root=project_root, bundle_root=bundle_root
        )
        _set_or_verify_hash(
            prompt, path_key="templatePath", hash_key="templateHash", label="policy template"
        )
        ...
        parse["schemaPath"] = _resolve_data_path(
            schema_file, project_root=project_root, bundle_root=bundle_root
        )
        _set_or_verify_hash(
            parse, path_key="schemaPath", hash_key="schemaHash", label="policy parse schema"
        )
        success = contract.setdefault("success", {})
        contract_file = str(success.get("contractFile") or "").strip()
        if contract_file:
            success["contractPath"] = _resolve_data_path(
                contract_file, project_root=project_root, bundle_root=bundle_root
            )
            _set_or_verify_hash(
                success,
                path_key="contractPath",
                hash_key="contractHash",
                label="policy success contract",
            )


def _resolve_success_paths(
    policy: dict[str, Any], *, project_root: Path, bundle_root: Path
) -> None:
    for contract in (policy.get("steps") or {}).values():
        success = contract.setdefault("success", {})
        contract_file = str(success.get("contractFile") or "").strip()
...
def _resolve_step_assets(step: str, assets: dict[str, Any], project_root: Path) -> dict[str, str]:
    ...
    files = {
        "skill": _resolve_required_file(
            skill_dir / "SKILL.md", project_root, required, "skill", step
        ),
        "workflow": _resolve_candidate_file(
            skill_dir, assets.get("workflowCandidates"), project_root, required, "workflow", step
        ),
        "instructions": _resolve_candidate_file(
            skill_dir,
            assets.get("instructionsCandidates"),
            project_root,
            required,
            "instructions",
            step,
        ),
        "checklist": _resolve_candidate_file(
            skill_dir, assets.get("checklistCandidates"), project_root, required, "checklist", step
        ),
        "template": _resolve_candidate_file(
            skill_dir, assets.get("templateCandidates"), project_root, required, "template", step
        ),
    }
    if not files["skill"]:
        files["workflow"] = ""
...
def _resolve_required_file(
    path: Path, project_root: Path, required: set[str], asset: str, step: str
) -> str:
    if path.is_file():
...
def _resolve_candidate_file(
    ...
    if asset in required:
        searched = ", ".join(
            str(skill_dir / str(name)) for name in candidates if isinstance(name, str) and name
        )
...
def _resolve_state_path(
    project_root: Path, path: Path, *, allow_outside: bool = True, label: str = "state file"
) -> Path:
...
def _set_or_verify_hash(
    payload: dict[str, Any], *, path_key: str, hash_key: str, label: str
) -> None:
