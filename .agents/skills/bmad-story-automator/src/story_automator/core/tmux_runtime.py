...
def verify_or_create_output(
    output_file: str, session_name: str, hash_value: str, *, project_root: str | None = None
) -> str:
...
def _spawn_runner(
    ...
    _write_private_text(
        paths.runner, _runner_file_content(paths, bash_path, command_shell, str(root)), 0o700
    )
...
def _legacy_claude_session_status(
    ...
    *,
    ...
    if bool(state["has_ever_been_active"]):
        ...
        session_state = (
            "just_started" if current_status_time != str(state["last_statusline_time"]) else "stuck"
        )
...
def _pane_snapshot(session: str) -> PaneSnapshot:
    ...
    return PaneSnapshot(
        True, pane_id, pane_pid, pane_dead, _safe_int(dead_status) if dead_status else None
    )
