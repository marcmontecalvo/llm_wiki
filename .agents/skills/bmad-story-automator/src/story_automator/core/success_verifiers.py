...
def resolve_success_contract(
    project_root: str, step: str, *, state_file: str | Path | None = None
) -> dict[str, Any]:
...
def create_story_artifact(
    *,
    ...
    raw_glob = str(
        config.get("glob") or "_bmad-output/implementation-artifacts/{story_prefix}-*.md"
    )
    expected = _parse_int(
        config.get("expectedMatches", 1), "success.config.expectedMatches", minimum=0
    )
...
def _story_artifact_path(project_root: str, story_prefix: str) -> Path | None:
    matches = sorted(
        (Path(project_root) / "_bmad-output" / "implementation-artifacts").glob(
            f"{story_prefix}-*.md"
        )
    )
...
def _resolve_artifact_glob(project_root: str, pattern: str) -> tuple[Path, str]:
    ...
    if raw.is_absolute():
        raise PolicyError(
            "success.config.glob must be relative to _bmad-output/implementation-artifacts"
        )
    ...
    try:
        ...
        raise PolicyError(
            "success.config.glob must stay within _bmad-output/implementation-artifacts"
        ) from exc
...
def _validate_review_contract(contract: dict[str, Any]) -> None:
    ...
    invalid_sources = sorted(
        {value for value in source_order if value not in ALLOWED_REVIEW_SOURCES}
    )
    if invalid_sources:
        raise PolicyError(
            f"review contract sourceOrder contains unknown sources: {', '.join(invalid_sources)}"
        )
