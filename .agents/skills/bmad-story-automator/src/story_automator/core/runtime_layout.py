...
def _project_root(project_root: str | Path | None = None) -> Path:
    return (
        Path(project_root or os.environ.get("PROJECT_ROOT") or os.getcwd()).expanduser().resolve()
    )
...
def _skill_present(skill_dir: Path, *, policy: bool = False) -> bool:
    ...
    return (skill_dir / "SKILL.md").is_file() or (
        skill_dir / "data" / "orchestration-policy.json"
    ).is_file()
...
def runtime_provider(project_root: str | Path | None = None) -> str:
    ...
    if inferred:
        return inferred
    root = _project_root(project_root)
    if (root / ".agents" / "skills" / STORY_SKILL_NAME).exists() or (
        root / ".codex" / "skills" / STORY_SKILL_NAME
    ).exists():
        return "codex"
    return "claude"
...
def active_marker_path(project_root: str | Path | None = None) -> Path:
    ...
    try:
        ...
        if skills_root.parent.name in {".claude", ".agents", ".codex"} and (
            has_story_skill or explicit_root or current_root
        ):
...
def _resolve_skill_relative_path(
    path_value: str, project_root: str | Path | None = None
) -> Path | None:
