...
def _codex_hook_message(changed: bool, trusted: bool) -> str:
    if changed:
        ...
        return (
            "Codex Stop hook configured in .codex/hooks.json and codex_hooks enabled in .codex/config.toml. "
            + suffix
        )
    if trusted:
...
def _ensure_json_stop_hook(
    ...
    *,
    ...
    update = _prepare_json_stop_hook(
        path, command=command, timeout=timeout, status_message=status_message
    )
...
def _prepare_json_stop_hook(
    ...
    *,
    ...
    if exists and not needs_update:
        return HookFileUpdate(
            result=HookInstallResult(changed=False, reason="already_configured", path=path)
        )
    if exists:
        return HookFileUpdate(
            result=HookInstallResult(
                changed=True, reason="hook_normalized", path=path, written=True
            ),
...
def _stop_hook_payload(
    *, command: str, timeout: int, status_message: str | None = None
) -> dict[str, Any]:
    hook: dict[str, Any] = {
        "type": "command",
        "command": command,
...
def _prepare_codex_hooks_feature(path: Path) -> HookFileUpdate:
    ...
    if features.get("codex_hooks") is True:
        return HookFileUpdate(
            result=HookInstallResult(changed=False, reason="already_enabled", path=path)
        )
    ...
    return HookFileUpdate(
        result=HookInstallResult(
            changed=True, reason="codex_hooks_enabled", path=path, written=True
        ),
        data=updated,
    )
