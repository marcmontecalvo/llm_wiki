...
def parse_story(epic_file: str | Path, story_id: str, rules_file: str | Path) -> dict[str, Any]:
    ...
    content_for_score = " ".join(
        part for part in [title, description, " ".join(acceptance_criteria)] if part
    ).strip()
    score = 0
    reasons: list[str] = []
...
def epic_complete(epic_file: str | Path, range_csv: str) -> dict[str, Any]:
    ...
    max_epic_story = max(
        story_ids, key=lambda value: tuple(int(part) for part in value.split(".", 1))
    )
    selected = [part.strip() for part in range_csv.split(",") if part.strip()]
    max_range_story = (
        max(selected, key=lambda value: tuple(int(part) for part in value.split(".", 1)))
        if selected
        else "0.0"
    )
    return {
        "ok": True,
        "epicComplete": max_range_story == max_epic_story,
        "maxEpicStory": max_epic_story,
    }
