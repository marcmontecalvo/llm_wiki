...
def check_epic_complete_action(args: list[str]) -> int:
    ...
    if state_file and file_exists(state_file):
        ...
        stories = [
            sid for sid in story_range if isinstance(sid, str) and sid.startswith(f"{epic}.")
        ]
        source = "state_file"
    else:
    ...
    if stories:
        stories = sorted(
            set(stories),
            key=lambda item: tuple(int(part) for part in item.replace("-", ".").split(".")[:2]),
        )
        last = stories[-1]
        print_json(
            {
                "ok": True,
                "isLastStory": story in {last, last.replace("-", ".")},
                "epic": int(epic),
                "storyId": story,
                "lastInEpic": last,
                "epicStoryCount": len(stories),
                "source": source,
            }
        )
        return 0
...
def get_epic_stories_action(args: list[str]) -> int:
    ...
    if state_file and file_exists(state_file):
        stories = [
            sid
            for sid in parse_frontmatter(read_text(state_file)).get("storyRange", [])
            if isinstance(sid, str) and sid.startswith(f"{epic}.")
        ]
        if stories:
            print_json(
                {
    ...
    if epic_file:
        stories = sorted(
            set(re.findall(rf"\b{re.escape(epic)}\.\d+", read_text(epic_file))),
...
def agents_build_action(args: list[str]) -> int:
    ...
    while idx < len(args):
        ...
        if idx + 1 < len(args):
            ...
            idx += 1
    if (
        not all(options.values())
        or not file_exists(options["state-file"])
        or not file_exists(options["complexity-file"])
    ):
        print_json(
            {
                "ok": False,
                "error": "missing_args" if not all(options.values()) else "file_not_found",
            }
        )
        return 1
    ...
    payload = {
        ...
        "epic": state_fields.get("epic", ""),
        "epicName": state_fields.get("epicName", ""),
        "createdAt": iso_now(),
        "stories": stories,
    }
...
def agents_resolve_action(args: list[str]) -> int:
    ...
    if (
        not options["story"]
        or not options["task"]
        or (not options["state-file"] and not options["agents-file"])
    ):
        print_json({"ok": False, "error": "missing_args"})
        return 1
    agents_path = options["agents-file"] or find_frontmatter_value(
        options["state-file"], "agentsFile"
    )
...
def parse_agent_config(raw: str) -> dict:
    ...
    return {
        ...
        if fallback_raw in {False, "false", "none", "null"}
        else (fallback_raw or "false"),
        "perTask": per_task,
    ...
    if task in config["perTask"]:
        ...
        if isinstance(entry, dict):
            ...
            if "fallback" in entry:
                fallback = (
                    "false"
                    if entry["fallback"] in {False, "false", "none", "null"}
                    else entry["fallback"]
                )
