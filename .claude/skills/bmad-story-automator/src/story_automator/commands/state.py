...
def cmd_build_state_doc(args: list[str]) -> int:
    ...
    text = re.sub(
        r"(?m)^customInstructions:.*$", lambda m: f"customInstructions: {custom_instructions}", text
    )
    ...
    for key, value in replacements.items():
        text = re.sub(
            rf"(?m)^{re.escape(key)}:.*$", lambda m, k=key, v=value: f"{k}: {json.dumps(v)}", text
        )
...
def cmd_validate_state(args: list[str]) -> int:
    ...
    required(
        "lastUpdated",
        lambda value: isinstance(value, str) and re.search(r"\d{4}-\d{2}-\d{2}T", value),
    )
