...
- [ ] Task 1: Create `PromotionEngine` in `src/llm_wiki/synthesis/scheduler.py:PromotionJob` (AC: 1, 2, 3, 4)
  - [ ] 1.1 New file: `src/llm_wiki/daemon/jobs/promotion.py` — `PromotionJob` daemon job
  - [ ] 1.2 Load `domains.yaml` for threshold config; parse per-domain or global defaults
  - [ ] 1.3 Iterate all pages across domains; group by entity identity (page title/key similarity)
  - [ ] 1.4 For entities appearing in ≥ 2 domains with average `confidence_score ≥ threshold` — flag as candidate
  - [ ] 1.5 Non-candidates are left untouched; no side effects
- [ ] Task 2: Implement shared page creation (AC: 2, 3)
  - [ ] 2.1 Create `wiki_system/shared/{entity-slug}.md` with frontmatter: `kind: entity`, `domains: [list]`, `promoted_at: timestamp`, `source_pages: [ids]`
  - [ ] 2.2 On each source domain page, write a tombstone: replace content with redirect to shared page ID
  - [ ] 2.3 Add backlink from shared page back to each original domain page
  - [ ] 2.4 Move original page file to `wiki_system/archive/{domain}/{original-slug}.md` (keep in archive, not delete)
  - [ ] 2.5 All file operations use atomic write (tmp + os.replace pattern from Story 1.1)
- [ ] Task 3: Wire into daemon scheduler (AC: 1, 4)
  - [ ] 3.1 Add `PromotionJob` to `WikiDaemon` scheduler with 24h interval (per architecture docs)
  - [ ] 3.2 Read threshold from `daemon.yaml` features block: `cross_domain_promotion: true`
  - [ ] 3.3 New config field: `PromotionConfig` in `models/config.py` with `min_domains: int = 2`, `min_confidence: float = 0.6`
  - [ ] 3.4 Threshold changes take effect on next job run — no restart needed beyond scheduler re-reading config
- [ ] Task 4: Make shared pages visible in search (AC: 5)
  - [ ] 4.1 `shared/` directory is indexed like other domain directories
  - [ ] 4.2 Shared pages appear in `WikiQuery.search()` and `WikiQuery.query()` results for any domain
  - [ ] 4.3 Shared page backlinks ensure they get authority_score (Story 3.1 integration)
- [ ] Task 5: Implement idempotent promotion (AC: 6)
  - [ ] 5.1 If a page in `shared/` is re-ingested: use existing merge strategy to update content in-place
  - [ ] 5.2 If promotion runs on an already-promoted entity: skip — check `shared/` for existing entity before promoting
- [ ] Task 6: Write tests (AC: 2, 3, 6)
  - [ ] 6.1 Unit: test PromotionJob scoring with known page counts across domains
  - [ ] 6.2 Unit: test threshold gating — below threshold, no promotion
  - [ ] 6.3 Unit: test config parsing from domains.yaml
  - [ ] 6.4 Integration: test full promotion flow — page creation, tombstone writing, shared page indexed
  - [ ] 6.5 Integration: test idempotent re-ingestion after promotion fails merge
